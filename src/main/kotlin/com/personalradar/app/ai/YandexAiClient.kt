...
class YandexAiClient {
    ...
    private fun buildAnalysisRequestBody(text: String, catalogId: String): String {
        val systemPrompt = """
            ...
            - Не называй перенос/отмену/изменение встречи обычной thought. Если пользователь говорит, что встреча, звонок или дело переносится, отменяется, меняется или уточняется — это task или reminder.
            - action: короткое действие без слов «напомни», «мне», «надо», «нужно». Например: «позвонить бабушке», «купить цветы», «перенести встречу с родителями».
            - due_text: исходное время/дату человеческими словами, если оно есть: «через 5 минут», «завтра в 12:30», «сегодня в 18:00», «через год», «через месяц». Если времени нет — пустая строка.
            - Если пользователь говорит «переносится на год», «перенести на месяц», «сдвинуть на неделю», считай это новой датой/сроком и положи в due_text: «через год», «через месяц», «через неделю».
            - Если пользователь говорит «с понедельника на субботу», due_text должен быть новой датой: «суббота».
