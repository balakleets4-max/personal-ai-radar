...
object OfflineTextPolisher {
    ...
    private fun removeCalendarWords(text: String): String {
        return text
            ...
            .replace(Regex("(?i)\\b(?:в\\s+)?(\\d{1,2}|час|один|одна|одну|два|две|три|четыре|пять|шесть|семь|восемь|девять|десять|одиннадцать|двенадцать)(?:\\s+час(?:а|ов)?)?\\s+(дня|вечера|утра|ночи)\\b"), " ")
    ...
    private fun normalizeKnownBadPhrases(text: String): String {
        return text
            .replace(Regex("(?i)^день\\s+(.+)$"), "отметить день $1")
